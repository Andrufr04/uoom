...
export default function Cart() {
    ...
    const sum = cart.items.reduce((s, ci) => s + ci.price, 0)
                ...
                <div className="col col-4 cart-order m-3 p-4">
                    ...
                    <div>
                        <div className="cart-cost">
                            ...
                            <div>{sum.toMoney()} грн</div>
                        </div>
                        <div className="sale">
                            {cart.items.reduce((s, ci) => s + ci.cnt, 0) > 2 &&
                                <div className="cart-cost">
                                    <div>Знижка:</div>
                                    <div>
                            ...
                            <div className="cart-cost cart-total-cost">
                                <div>Разом:</div>
                                <div className="cart-total-price">{cart.items.reduce((s, ci) => s + ci.cnt, 0) > 2
                                    ? parseFloat((sum * 0.85).toFixed(2)).toMoney()
                                    : (sum).toMoney()} грн</div>
                            </div>
                        </div>
                    </div>
